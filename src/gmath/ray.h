...
namespace gmath
{
    ...
    struct ray
    {
        ...
        constexpr ray(position<space> const& start, direction<space> const& direction) noexcept
            : m_start(start), m_direction(direction) {}
        constexpr ray(position<space> const& start, direction<space> const& direction, float minT, float maxT, int depth) noexcept 
            : m_start(start), m_direction(direction), m_minT(minT), m_maxT(maxT), m_depth(depth) {}
        constexpr ray(position<space> const& start, direction<space> const& direction, ray const& parent) noexcept
        ...
        constexpr direction<space> const& dir() const noexcept { return m_direction; }
        ...
        direction<space>& dir() noexcept { return m_direction; }
