...
namespace gscene
{
    void gscene::mesh_resource::load()
    {
        ...
        m_vertices_position.reserve(mesh.mNumVertices);
        m_vertices_normal.reserve(mesh.mNumVertices);
        for (std::size_t i = 0; i < mesh.mNumVertices; ++i)
        {
            auto const& vertice = mesh.mVertices[i];
            m_vertices_position.emplace_back(vertice.x, vertice.y, vertice.z);
            auto const& normal = mesh.mNormals[i];
            m_vertices_normal.emplace_back(normal.x, normal.y, normal.z);
        }

        m_faces_index.reserve(mesh.mNumFaces);
        for (std::size_t i = 0; i < mesh.mNumFaces; ++i)
        {
            auto const& triangle_face = mesh.mFaces[i];
            m_faces_index.emplace_back(triangle_face.mIndices[0], triangle_face.mIndices[1], triangle_face.mIndices[2], m_vertices_position);
        }
    }
